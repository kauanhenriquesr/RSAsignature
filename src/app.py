...
def assina(file_path, n, private_key):
    ...
    with open(file_path[0:-4]+"_assinatura.sig", "w") as f:
        ...
        print("-"*30)
        print(f"Assinatura salva em {file_path[0:-4]+"_assinatura.sig"} com sucesso!")
...
def main(args):
    if args == [] or args[0] in ["-h", "--help"]:
        ...
        try: private_key = int(open("assets/private_key.key").read())
            ...
            print(f"Arquivo de chave privada não encontrado. Por favor, gere as chaves primeiro.")
            return 1
        try:
